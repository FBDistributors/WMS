import ExcelJS from 'exceljs'

import type { ReceiptListExportContext } from './receiptListExport'

const DETAILED_COL_COUNT = 12
const GROUPED_COL_COUNT = 5
const ARGB_BLUE = 'FF1E40AF'
const ARGB_WHITE = 'FFFFFFFF'
const ARGB_LABEL_BG = 'FFF1F5F9'
const ARGB_BORDER = 'FFCBD5E1'
const ARGB_TITLE = 'FF0F172A'
const ARGB_META = 'FF475569'

function excelThinBorder(): Partial<ExcelJS.Borders> {
  const edge: Partial<ExcelJS.Border> = { style: 'thin', color: { argb: ARGB_BORDER } }
  return { top: edge, left: edge, bottom: edge, right: edge }
}

function safeSheetName(title: string): string {
  const cleaned = title.replace(/[\\/*?:[\]]/g, '_').trim()
  return (cleaned || 'Receipts').slice(0, 31)
}

function setDetailedDataCell(cell: ExcelJS.Cell, colIndex: number, val: string | number): void {
  if (colIndex === 7) {
    const n = typeof val === 'number' ? val : Number(val)
    cell.value = Number.isFinite(n) ? n : val
    if (typeof cell.value === 'number') cell.numFmt = '0'
    return
  }
  if (colIndex === 8) {
    if (typeof val === 'number' && Number.isFinite(val)) {
      cell.value = val
      cell.numFmt = '0'
    } else {
      cell.value = val === null || val === undefined ? '' : String(val)
    }
    return
  }
  cell.value = val === null || val === undefined ? '' : val
}

function setGroupedDataCell(cell: ExcelJS.Cell, colIndex: number, val: string | number): void {
  if (colIndex === 3) {
    const n = typeof val === 'number' ? val : Number(val)
    cell.value = Number.isFinite(n) ? n : val
    if (typeof cell.value === 'number') cell.numFmt = '0'
    return
  }
  if (colIndex === 4) {
    if (typeof val === 'number' && Number.isFinite(val)) {
      cell.value = val
      cell.numFmt = '0'
    } else {
      cell.value = val === null || val === undefined ? '' : String(val)
    }
    return
  }
  cell.value = val === null || val === undefined ? '' : val
}

function writeMetaBlock(
  ws: ExcelJS.Worksheet,
  colCount: number,
  title: string,
  metaRows: [string, string][]
): void {
  ws.mergeCells(1, 1, 1, colCount)
  const titleCell = ws.getCell(1, 1)
  titleCell.value = title
  titleCell.font = { bold: true, size: 16, color: { argb: ARGB_TITLE } }
  titleCell.alignment = { vertical: 'middle' }
  ws.getRow(1).height = 28

  metaRows.forEach(([label, value], i) => {
    const rowNum = 2 + i
    const labelCell = ws.getCell(rowNum, 1)
    labelCell.value = label
    labelCell.font = { bold: true, color: { argb: ARGB_META } }
    labelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ARGB_LABEL_BG } }
    labelCell.border = excelThinBorder()
    labelCell.alignment = { vertical: 'middle' }

    ws.mergeCells(rowNum, 2, rowNum, colCount)
    const valueCell = ws.getCell(rowNum, 2)
    valueCell.value = value
    valueCell.font = { color: { argb: ARGB_TITLE } }
    valueCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ARGB_WHITE } }
    valueCell.border = excelThinBorder()
    valueCell.alignment = { vertical: 'middle', wrapText: true }
    ws.getRow(rowNum).height = 20
  })
}

function addReceiptListDetailedSheet(wb: ExcelJS.Workbook, ctx: ReceiptListExportContext): void {
  const { labels, filterSummaryLines, rows, receiptCount, lineCount } = ctx
  const ws = wb.addWorksheet(safeSheetName(labels.sheetDetailed), {
    views: [{ state: 'frozen', ySplit: 6 }],
  })

  ws.columns = [
    { width: 18 },
    { width: 12 },
    { width: 16 },
    { width: 18 },
    { width: 12 },
    { width: 16 },
    { width: 40 },
    { width: 8 },
    { width: 8 },
    { width: 14 },
    { width: 12 },
    { width: 12 },
  ]

  writeMetaBlock(ws, DETAILED_COL_COUNT, labels.listTitle, [
    [labels.filterSummary, filterSummaryLines.join('; ') || '—'],
    [labels.receiptsCount, String(receiptCount)],
    [labels.linesCount, String(lineCount)],
  ])

  const headerRowNum = 6
  const headers = [
    labels.colDocNo,
    labels.status,
    labels.receivedBy,
    labels.receivedAt,
    labels.colCode,
    labels.colBarcode,
    labels.colProduct,
    labels.colQty,
    labels.colQoldiq,
    labels.colBatch,
    labels.colExpiry,
    labels.colLocation,
  ]
  const headerRow = ws.getRow(headerRowNum)
  headerRow.height = 22
  headers.forEach((text, colIndex) => {
    const cell = headerRow.getCell(colIndex + 1)
    cell.value = text
    cell.font = { bold: true, color: { argb: ARGB_WHITE } }
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ARGB_BLUE } }
    cell.border = excelThinBorder()
    cell.alignment = { vertical: 'middle', horizontal: 'left', wrapText: true }
  })

  rows.forEach((row, lineIndex) => {
    const rowNum = headerRowNum + 1 + lineIndex
    const dataRow = ws.getRow(rowNum)
    dataRow.height = 20
    const values: (string | number)[] = [
      row.docNo,
      row.status,
      row.receivedBy,
      row.receivedAt,
      row.code,
      row.barcode,
      row.productName,
      row.qty,
      row.qoldiq,
      row.batch,
      row.expiry,
      row.location,
    ]
    values.forEach((val, colIndex) => {
      const cell = dataRow.getCell(colIndex + 1)
      setDetailedDataCell(cell, colIndex, val)
      cell.font = { color: { argb: ARGB_TITLE } }
      cell.border = excelThinBorder()
      cell.alignment = {
        vertical: 'middle',
        horizontal: colIndex === 7 || colIndex === 8 ? 'right' : 'left',
        wrapText: colIndex === 6,
      }
    })
    if (lineIndex % 2 === 1) {
      for (let c = 1; c <= DETAILED_COL_COUNT; c++) {
        const cell = dataRow.getCell(c)
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF8FAFC' } }
      }
    }
  })
}

function addReceiptListByProductSheet(wb: ExcelJS.Workbook, ctx: ReceiptListExportContext): void {
  const { labels, filterSummaryLines, groupedRows, receiptCount, productsCount } = ctx
  const ws = wb.addWorksheet(safeSheetName(labels.sheetByProduct), {
    views: [{ state: 'frozen', ySplit: 6 }],
  })

  ws.columns = [
    { width: 14 },
    { width: 18 },
    { width: 48 },
    { width: 12 },
    { width: 10 },
  ]

  writeMetaBlock(ws, GROUPED_COL_COUNT, labels.listTitleByProduct, [
    [labels.filterSummary, filterSummaryLines.join('; ') || '—'],
    [labels.receiptsCount, String(receiptCount)],
    [labels.productsCount, String(productsCount)],
  ])

  const headerRowNum = 6
  const headers = [
    labels.colCode,
    labels.colBarcode,
    labels.colProduct,
    labels.colGroupedTotalQty,
    labels.colQoldiq,
  ]
  const headerRow = ws.getRow(headerRowNum)
  headerRow.height = 22
  headers.forEach((text, colIndex) => {
    const cell = headerRow.getCell(colIndex + 1)
    cell.value = text
    cell.font = { bold: true, color: { argb: ARGB_WHITE } }
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ARGB_BLUE } }
    cell.border = excelThinBorder()
    cell.alignment = { vertical: 'middle', horizontal: 'left', wrapText: true }
  })

  groupedRows.forEach((row, lineIndex) => {
    const rowNum = headerRowNum + 1 + lineIndex
    const dataRow = ws.getRow(rowNum)
    dataRow.height = 20
    const values: (string | number)[] = [
      row.code,
      row.barcode,
      row.productName,
      row.totalQty,
      row.qoldiq,
    ]
    values.forEach((val, colIndex) => {
      const cell = dataRow.getCell(colIndex + 1)
      setGroupedDataCell(cell, colIndex, val)
      cell.font = { color: { argb: ARGB_TITLE } }
      cell.border = excelThinBorder()
      cell.alignment = {
        vertical: 'middle',
        horizontal: colIndex === 3 || colIndex === 4 ? 'right' : 'left',
        wrapText: colIndex === 2,
      }
    })
    if (lineIndex % 2 === 1) {
      for (let c = 1; c <= GROUPED_COL_COUNT; c++) {
        const cell = dataRow.getCell(c)
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF8FAFC' } }
      }
    }
  })
}

export async function buildStyledReceiptListWorkbook(
  ctx: ReceiptListExportContext
): Promise<ExcelJS.Workbook> {
  const wb = new ExcelJS.Workbook()
  wb.creator = 'WMS'
  addReceiptListDetailedSheet(wb, ctx)
  addReceiptListByProductSheet(wb, ctx)
  return wb
}
