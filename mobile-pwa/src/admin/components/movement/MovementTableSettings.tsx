...
import type { MovementTableConfig } from '../../hooks/useMovementTableConfig'

type Option = { id: string; label: string }

type MovementTableSettingsProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  config: MovementTableConfig
  columns: Option[]
  onSave: (config: MovementTableConfig) => void
  onReset: () => void
}
...
export function MovementTableSettings({
  open,
  onOpenChange,
  config,
  columns,
      ...
      <div className="relative ml-auto flex h-full w-full max-w-[520px] flex-col bg-white shadow-2xl dark:bg-slate-950">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {t('admin:movement_page.table.settings_title')}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {t('admin:movement_page.table.columns_hint')}
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
          <section>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
              {t('admin:movement_page.table.columns_title')}
            </h3>
        ...
        <div className="border-t border-slate-200 px-6 py-4 dark:border-slate-800">
          <div className="flex items-center justify-end gap-3">
            ...
            >
              {t('admin:movement_page.table.reset_default')}
            </Button>
            <Button
              onClick={() => {
            ...
            >
              {t('admin:movement_page.table.save_settings')}
            </Button>
          </div>
        </div>
      </div>
