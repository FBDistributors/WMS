...
import type { KamomatTableConfig } from '../../hooks/useKamomatTableConfig'

type Option = { id: string; label: string }

type KamomatTableSettingsProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  config: KamomatTableConfig
  columns: Option[]
  onSave: (config: KamomatTableConfig) => void
  onReset: () => void
}
...
export function KamomatTableSettings({
  open,
  onOpenChange,
  config,
  columns,
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {t('kamomat:table.settings_title')}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {t('kamomat:table.columns_hint')}
            </p>
          </div>
          <Button
            variant="ghost"
