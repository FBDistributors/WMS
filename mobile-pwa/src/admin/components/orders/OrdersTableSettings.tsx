...
export function OrdersTableSettings({
  open,
  onOpenChange,
  config,
  columns,
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto flex h-full w-full max-w-[520px] flex-col bg-white shadow-2xl dark:bg-slate-950">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              {t('orders:table.settings_title')}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {t('orders:table.columns_hint')}
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
          ...
          <section>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
              {t('orders:table.search_title')}
            </h3>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              {t('orders:table.search_hint')}
            </p>
