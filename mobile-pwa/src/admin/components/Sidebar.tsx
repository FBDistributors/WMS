...
const MENU_ITEMS: Array<MenuItem & { key: string }> = [
  ...
  { key: 'orders_diller', label: 'Tashkiliy harakatlar', path: '/admin/orders-diller', icon: ClipboardList, required: 'orders:read' },
  ...
  { key: 'kamomat', label: 'Inventarizatsiya', path: '/admin/kamomat', icon: FileText, required: 'audit:read' },
]
    ...
    >
      <div className="mb-6 flex items-center justify-between gap-2">
        <Link to="/admin" className="flex items-center gap-2" onClick={onNavigate}>
          ...
          {!collapsed && (
            <span className="text-sm font-semibold leading-tight text-slate-900 dark:text-slate-100">{BRAND.name}</span>
          )}
        </Link>
        <Button
          variant="secondary"
