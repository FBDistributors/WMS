...
export function PickListReadOnlyDetail({ documentId }: PickListReadOnlyDetailProps) {
  ...
  const docStatusLabel = (raw: string) => {
    ...
    return t(`picking:doc_status.${key}`, { defaultValue: raw })
  }
  ...
  const title = data.order_number
    ? t('picking:order_number_display', { number: data.order_number })
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-center justify-between text-sm text-slate-600 dark:text-slate-400">
          <span>{t('picking:progress_picked', { picked: progress.picked, total: progress.total })}</span>
            ...
            <tbody>
              {data.lines.map((line) => (
                ...
                >
                  <td className="max-w-[min(360px,40vw)] px-3 py-3 text-slate-900 dark:text-slate-100 sm:px-4">
                    ...
                    {line.batch ? (
                      <p className="mt-0.5 text-xs text-slate-500">
                        {t('picking:batch_label')}: {line.batch}
                        {line.expiry_date ? ` · ${t('picking:expiry_label')}: ${line.expiry_date}` : ''}
                      </p>
                    ) : null}
                  </td>
                  ...
                  <td className="max-w-[140px] truncate px-3 py-3 text-slate-600 dark:text-slate-400 sm:px-4" title={line.sku ?? line.barcode ?? ''}>
