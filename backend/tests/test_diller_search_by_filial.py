...
def test_diller_search_accepts_status_and_movement_note_fields(
    client: TestClient, db_session: Session
) -> None:
    admin = _mk_user(db_session, username=f"adm-diller-{uuid.uuid4().hex[:8]}", role="warehouse_admin")

    org = SettingsOrganization(org_id="7000002", name="Дилер Наманган (Шухрат)")
    db_session.add(org)
    db_session.flush()
    ...
    order = Order(
        source="diller",
        source_external_id=f"mfm:search-{uuid.uuid4().hex[:8]}",
        order_number="MV-SEARCH-2",
        filial_id="7000002",
        to_filial_code="7000002",
        movement_note="transfer to дилер",
    )
    order.wms_state = OrderWmsState(status="W")
    db_session.add(order)
    db_session.commit()
    ...
    try:
        res = client.get(
            ...
            params={
                ...
                "filial_id": "all",
                "q": "дилер",
                "search_fields": "status,to_filial,movement_note",
            },
        )
    finally:
        app.dependency_overrides.pop(get_current_user, None)
